...
``docs/examples/assets/`` so MkDocs can embed them in ``<iframe>``s. Plotly
output is pointed at the CDN (instead of inlining ~4.5 MB of plotly.js per
file) to keep the site light.
...
# Demos that emit standalone HTML we embed. (holoviews_demo and
# rug_borderless_demo are skipped: the former emits PNGs, the latter pops a
# browser window.)
DEMOS = [
    "svg_demo",
    "summary_demo",
